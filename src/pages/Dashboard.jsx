...
function Dashboard({stats,classes,school,schools,offices,onSelectSchool,onDeleteOffice}){
 ...
 const overallSummaries = schools.map(summarizeSchool);
 const overallSchools = schools.length;
 const overallStudents = overallSummaries.reduce((sum, x) => sum + x.students, 0);
 const overallScored = overallSummaries.reduce((sum, x) => sum + x.scored, 0);
 const overallPassed = overallSummaries.reduce((sum, x) => sum + x.passed, 0);
 ...
 const openSchool=schoolId=>{const target=schools.find(item=>item.id===schoolId);setSelectedOfficeId(target?.officeId||'unassigned');onSelectSchool(schoolId);setDashboardView('school')};
  return <>
   ...
   <div className="card insight-panel improve"><div className="insight-panel-head"><span><LayoutDashboard/><b>ห้องที่ควรพัฒนา</b></span><strong>{insights.roomsToImprove.length}</strong></div><small className="insight-rule">ผู้ผ่านเกณฑ์ต่ำกว่า 60% หรือยังไม่มีผลสอบ</small><div className="insight-list">{insights.roomsToImprove.map(item=><button key={item.classroom_id} onClick={()=>openSchool(item.school_id)}><span><b>{item.classroom_name}</b><small>{item.school_name} · เข้าคะแนน {item.scored}/{item.students} คน</small></span><em className={Number(item.scored)===0?'pending':''}>{Number(item.scored)===0?'ยังไม่มีคะแนน':`${item.pass_percent}%`}</em></button>)}{!insights.roomsToImprove.length&&<p>ทุกห้องมีผู้ผ่านเกณฑ์อย่างน้อย 60%</p>}</div></div>
